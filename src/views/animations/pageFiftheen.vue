...
onMounted(()=>{
	...
	function Eye( canvas, x, y, scale, time ) {
		...
		this.render = function( mouse ) {
			...
			// Eye top/bottom
			const et = { x: this.x, y: this.y - ( this.size * ( 0.5 + ( this.exposure.top * this.exposure.current ) ) ) }
			const eb = { x: this.x, y: this.y - ( this.size * ( 0.5 - ( this.exposure.bottom * this.exposure.current ) ) ) }
		
			// Eye inner shadow top
			const eit = { x: this.x, y: this.y - ( this.size * ( 0.5 + ( ( 0.5 - this.tiredness ) * this.exposure.current ) ) ) }
